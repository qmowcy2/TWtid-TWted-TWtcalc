...
TWtid.$border_down.hide();TWtid.$border_left.hide();TWtid.$border_right.hide();TWtid.$cur_block=null;c=a?a.size():0;if(readOnly||0==c||"off"==b||1==c&&a.is(".tabset,.toolbar,.subtitle,.tiddler.tagged,.tagInfo,.tidTags"))return!1;var c=1==c&&a.is("table")?a.closest(".tedTable"):a,c=TWtid.element_box(c),d=TWtid.css_size("font-size",a);return c.height>=d&&c.top>d&&c.left>d?(TWtid.$cur_block=a,TWtid.$border_up.css({left:c.left,top:c.top,width:c.width}).show(),TWtid.$border_down.css({left:c.left,top:c.top+
c.height,width:c.width}).show(),TWtid.$border_left.css({left:c.left,top:c.top,height:c.height}).show(),TWtid.$border_right.css({left:c.left+c.width,top:c.top,height:c.height}).show(),!0):!1},mousemove:function(a){var a=a||window.event,b=jQuery(document.elementFromPoint(a.clientX,a.clientY));TWtid.$btn_lt||(TWtid.$btn_lt=TWtid.create_button("H","Scroll to A0",function(){TWtid.button_active(jQuery(this))&&TWtid.$cur_block.parent().animate({scrollLeft:0,scrollTop:0})}),TWtid.$btn_lt=TWtid.$btn_lt.add(TWtid.create_button("T",
"Transpose",function(){if(!(0<jQuery(this).parent().find(".tedTopt").size())){var a=TWtid.$cur_block,b=TWtid.wrapper_tiddler(a),e=TWtid.element_index(a,b),a=b.tiddler.text.split("\n"),e=TWtid.table_first_line(a,e,b.title),f=TWtid.table_last_line(a,e);TWtid.transpose_table(a,e,f);TWtid.save_text(b.tiddler,a);TWtid.refresh_wrapper(b,!0)}})),TWtid.$btn_lt.appendTo(this).hide());TWtid.inactive_elem(b)||((a=TWtid.block_element(b,a))?(!TWtid.$cur_blk||TWtid.$cur_blk[0]!=a[0])&&TWtid.focus(a,"on"):TWtid.focus())},
wrapper_closed:function(a){a=a.closest(".sliderPanel");return 0<a.size()&&"none"==a[0].style.display},hasClass:function(a,b,c,d){if(a.hasClass(c))return!0;if(!d||!d.tiddler)return!1;if(config.macros.sortableGridPlugin){a=d.tiddler.text.split("\n");d=TWtid.table_first_line(a,b,d.title);for(b=TWtid.table_last_line(a,d);d<=b;d++)if(/[kK]$/.test(a[d])&&-1<a[d].indexOf(c))return!0}return!1},prepare_table:function(a,b,c,d){if(TWtid.wrapper_closed(a)||0<a.closest(".tedTable").size())return!1;var e=a.parent();
e.is("[id^=tedT]")&&(e=e.parent().parent().find("div[id*=tedTCdiv]"));TWtid.css_size(e.css("line-height"));TWtid.init_table_rows(a,!0,b,c.title,d);config.options.chkTWtidEnabled&&(TWtid.reformat_table(a,b,c.title),a[0].tHead&&(config.tableSorting?TWtid.sortTable||(TWtid.sortTable=config.tableSorting.sortTable,config.tableSorting.sortTable=function(a,b){TWtid.sort(a,b)}):config.macros.sortableGridPlugin&&!TWtid.ts_resortTable&&(TWtid.ts_resortTable=config.macros.sortableGridPlugin.ts_resortTable,config.macros.sortableGridPlugin.ts_resortTable=
function(a){TWtid.sort(a)})));TWtid.fix_rows(a,!0).fix_cols(a,!0);return config.options.chkTWtidEnabled},closest_block:function(a,b,c,d){for(var e=c.clientX,c=c.clientY+d,f,b=0>d?0:b.top+b.height,h=c-b;0<d&&0>h||0>d&&0<h;){if(f=document.elementFromPoint(e,c))if((f=TWtid.block_element(jQuery(f)))&&f[0]!=a[0])break;f=null;c+=d;h=c-b}return f&&!f.is(".tagged,.tagInfo,.tidTags")?f:a},closest_blocks:function(a,b){var c=TWtid.css_size("line-height",a),d=[],e=TWtid.element_box(a);d[0]=TWtid.closest_block(a,
e,b,-c)[0];d[1]=TWtid.closest_block(a,e,b,c)[0];d[2]=a[0];return jQuery.contains(a[0],d[0])||jQuery.contains(a[0],d[1])?jQuery(d):null},is_block_element:function(a){switch(a[0].nodeName){case "DIV":return!0;case "TABLE":return!0;case "LI":case "BLOCKQUOTE":case "PRE":case "H1":case "H2":case "H3":case "H4":case "H5":case "H6":return 0==a.closest("th,td").size()}return!1},block_element:function(a,b){var c=null;if(TWtid.is_wrapper(a))return b&&(c=TWtid.closest_blocks(a,b)),c?c:a;if(TWtid.is_block_element(a))return a;
if(a.is("a.tab"))return a.parent();if((c=TWtid.table_element(a))&&0<c.size())return c;if(a.is("DIV")&&TWtid.$cur_block&&TWtid.$cur_block.is("TABLE")){var c=TWtid.$cur_block.closest(".tedTable"),d=TWtid.element_offset(c);if(b.pageX>=d.left&&b.pageY>=d.top&&b.pageX<=d.left+c.width()&&b.pageY<=d.top+c.height())return TWtid.$cur_block}c=a.closest("h1,h2,h3,h4,h5,h6");if(0<c.size())return c;c=a.closest("li");if(0<c.size())return c;c=a.closest("blockquote");if(0<c.size())return c;c=a.closest("pre");if(0<
c.size())return c;d=null;c=a.closest("span");if(TWtid.is_wrapper(c))return b&&(d=TWtid.closest_blocks(c,b)),d?d:c;c=a.closest("div");return TWtid.is_wrapper(c)?(b&&(d=TWtid.closest_blocks(c,b)),d?d:c):0<c.size()?c:null},apparent_dim:function(a){switch(a[0].nodeName){case "TABLE":config.options.chkTWtidEnabled&&(a=a.parent());case "PRE":case "BLOCKQUOTE":return{W:a.width(),H:a.height()};case "SPAN":case "DIV":if(a.is(".sliderPanel,.tabContents,[tiddler]"))return{W:a.width(),H:a.height()}}var b=0<a.find("span").size()?
a.parent():a,c=jQuery("<span></span>");config.browser.isIE?c.html(a.html()):TWtid.copy_html(c,a);c.appendTo(b);a={W:c.outerWidth(!0),H:c.outerHeight(!0)};c.remove();return a},find_text_nodes:function(a){return a.contents().filter(function(){return 3==this.nodeType})},find_elements:function(a,b,c){var d=c?a.children(b):a.find(b),a=a.is(".viewer")?a:a.find("div.viewer");0<a.size()&&(a=c?a.children("div.sliderPanel \t\t\t\t\t\t\t,div.tabContents \t\t\t\t\t\t\t,span[tiddler]"):a.find("div.sliderPanel \t\t\t\t\t\t\t,div.tabContents \t\t\t\t\t\t\t,span[tiddler]"),
0<a.size()&&(d=d.not(c?a.children(b):a.find(b))));return d},find_tables:function(a){return this.find_elements(a,"table")},find_headers:function(a){return this.find_elements(a,"h1,h2,h3,h4,h5,h6")},find_list_items:function(a){var b=this.find_elements(a,"li").not(".tagged li, .tidTags li"),c=jQuery(b[0]);-1<c.text().indexOf("not tagging")&&(b=b.not(c));return b.not(a.find("td li,th li"))},prepare_elements:function(a,b){if(0!=a.closest("#displayArea").size()&&b&&b.tiddler&&!store.isShadowTiddler(b.tiddler.title)){var c=
b.tiddler.text.split("\n");TWtid.find_tables(a).each(function(a,e){var f=jQuery(e);0==f.closest(".syntaxhighlighter").size()&&TWtid.prepare_table(f,{rIndex:a,dIndex:-1},b,c)})}},OfficialExample:function(a){return a?/^TWtid--Example/.test(a):!1},after_math:function(a){var b=jQuery(a[1]);switch(a[1].nodeName){case "TD":case "TH":a=b.closest("table");TWtid.fix_rows_adjust(a).fix_cols_adjust(a);break;case "SPAN":case "DIV":0<b.closest("#displayArea").size()&&TWtid.find_tables(b).each(function(){var a=
jQuery(this);TWtid.fix_rows_adjust(a).fix_cols_adjust(a)})}},check_MathJax:function(){config.extensions.MathJax&&("undefined"!==typeof MathJax&&story.displayTiddler==config.extensions.MathJax.displayTiddler)&&(story.displayTiddler=config.extensions.MathJax.displayTiddler_old,MathJax.Hub.Queue(["Typeset",MathJax.Hub]),MathJax.Hub.Register.MessageHook("End Process",TWtid.after_math))},slider_close:function(){},header_title:function(a,b,c){var d=a.text(),e=config.macros.foldHeadings.hidelabel;-1==d.indexOf(e)&&
(e=config.macros.foldHeadings.showlabel,-1==d.indexOf(e)&&(e=""));d=config.textPrimitives.sectionSeparator+(e?d.substring(e.length).trim():d);if(!c){b||(b=TWtid.wrapper_tiddler(a));b=TWtid.direct_wrapper(a,b);b=TWtid.find_headers(b);a=b.index(a);for(e=c=0;e<a;e++){var f=jQuery(b.get(e));TWtid.header_title(f,null,!0)==d&&c++}0<c&&(d+="?"+c)}return d},lines_of_text:function(a,b,c,d){if(void 0===d){d=c>=b?a[b]:"";for(b+=1;b<=c;b++)d+="\n"+a[b];return d}d?a.splice(b,c-b+1,d):a.splice(b,c-b+1)},header_content:function(a,
b){b||(b=TWtid.wrapper_tiddler(a));var c=TWtid.element_index(a,b),d=b.tiddler.text.split("\n"),e=TWtid.header_first_line(d,c,b.title),c=TWtid.header_first_line(d,{rIndex:-1,dIndex:c.dIndex+1},b.title);-1==c&&(c=d.length);for(var f=d[e+1],e=e+2;e<c;e++)f+="\n"+d[e];return f},header_to_skip:function(a){var b=a.lastIndexOf("?");return 0<b&&b<a.length-1?1*a.substring(b+1):0},remove_header_tail:function(a){var b=a.lastIndexOf("?");0<b&&(b<a.length-1&&!/[\D]/.test(a.substring(b+1)))&&(a=a.substring(0,b));
return a},pre_header_click:null,header_click:function(){TWtid.pre_header_click.apply(this,arguments);var a=jQuery(this),b=TWtid.wrapper_tiddler(a);b.title+=TWtid.header_title(a,b);TWtid.prepare_wrapper(a.next(),b)},prepare_wrapper:function(a,b){function c(a){if("undefined"!==typeof MathJax){var b=a.siblings(".title");0<b.size()&&MathJax.Hub.Queue(["Typeset",MathJax.Hub,b[0]]);MathJax.Hub.Queue(["Typeset",MathJax.Hub,a[0]])}}a.is(".sliderPanel")&&"none"==a[0].style.display||(config.macros.foldHeadings&&
a.find("h1.foldable, h2.foldable, h3.foldable\t\t\t\t\t\t, h4.foldable, h5.foldable").each(function(a,b){TWtid.pre_header_click||(TWtid.pre_header_click=b.onclick);b.onclick=TWtid.header_click}),TWtid.check_MathJax(),b||(b=this.wrapper_tiddler(a)),config.options.chkTWtidEnabled?(TWtid.prepare_elements(a,b),c(a)):(c(a),TWtid.prepare_elements(a,b)))},pre_refreshTiddler:null,refreshTiddler:function(a,b,c,d,e){var f=jQuery(TWtid.pre_refreshTiddler.apply(this,arguments)),h=f.find(".viewer");0==h.size()&&
(h=f);TWtid.prepare_wrapper(h);f.closest("div[id=tiddlerDisplay]").unbind("mousemove",TWtid.mousemove).bind("mousemove",TWtid.mousemove);return f[0]},pre_popup_show:null,popup_show:function(a,b,c){TWtid.pre_popup_show.apply(this,arguments);var d=Popup.stack[Popup.stack.length-1];TWtid.prepare_wrapper(jQuery(d.popup));return d},pre_tabs_switchTab:null,tabs_switchTab:function(a,b){TWtid.pre_tabs_switchTab.apply(this,arguments);var c=jQuery(a);0<c.closest("div[id=tiddlerDisplay]").size()&&(c=c.next(),
TWtid.prepare_wrapper(c),config.tableSorting&&config.tableSorting.refresh&&config.tableSorting.refresh(c[0]))},pre_tiddler_transclude:null,tiddler_transclude:function(a,b,c){TWtid.pre_tiddler_transclude.apply(this,arguments);var d=jQuery(a);0<d.closest("div[id=tiddlerDisplay]").size()&&TWtid.prepare_wrapper(d)},pre_slider_handler:null,slider_handler:function(a,b,c){TWtid.pre_slider_handler.apply(this,arguments);var d=jQuery(a);0<d.closest("div[id=tiddlerDisplay]").size()&&TWtid.prepare_wrapper(d.find("div[tiddler]"))},
pre_onClickSlider:null,onClickSlider:function(){TWtid.pre_onClickSlider.apply(this,arguments);TWtid.prepare_wrapper(jQuery(this).next())},refresh_wrapper:function(a,b){var c=jQuery(window),d=c.scrollLeft(),e=c.scrollTop();if(a.jquery)var f=TWtid.wrapper_tiddler(a),a={$wrapper:a,tiddler:f.tiddler,title:f.title};a.$wrapper.each(function(c,d){var e=jQuery(d),f=e.find(".viewer");0==f.size()&&(f=e);if(b){var m=TWtid.title_from_wrapper(e),e=m?TWtid.tiddler_section(m)?store.getTiddlerText(m):a.tiddler.text:
TWtid.header_content(e.prev(),a);f.empty();wikify(e,f[0]);TWtid.$cur_block=null}config.tableSorting&&config.tableSorting.refresh&&config.tableSorting.refresh(f[0]);TWtid.prepare_wrapper(f,a)});c.scrollLeft(d).scrollTop(e)},pre_go:null,go:function(a,b){var c=jQuery(a).closest("div[id=tiddlerDisplay]");TWtid.wrapper_tiddler(c);var d=config.macros.snapshot.defaultFilename,e=config.options.txtTWtidSnapshotFile;e?"tiddler.title"==e?e=TWtid.find_tiddler(c).title+".html":-1==e.indexOf(".html")&&(e+=".html"):
e=d;c=config.options.chkTWtidSnapshotHideBtn?c.find("a[id^=tedTbtn], a[snapID]"):null;config.macros.snapshot.defaultFilename=e;c&&c.hide();e=TWtid.pre_go.apply(this,arguments);c&&c.show();config.macros.snapshot.defaultFilename=d;return e}};
//}}}
// @@
