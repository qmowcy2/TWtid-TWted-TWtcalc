...
TWtid.$border_down.hide();TWtid.$border_left.hide();TWtid.$border_right.hide();TWtid.$cur_block=null;c=a?a.size():0;if(readOnly||0==c||"off"==b||1==c&&(a.is(".tabset")||a.prev().is(".title")))return!1;var c=1==c&&a.is("table")?a.closest(".tedTable"):a,c=TWtid.element_box(c),d=TWtid.css_size("line-height",a);return c.height>=d&&c.top>d&&c.left>d?(TWtid.$cur_block=a,TWtid.$border_up.css({left:c.left,top:c.top,width:c.width}).show(),TWtid.$border_down.css({left:c.left,top:c.top+c.height,width:c.width}).show(),
TWtid.$border_left.css({left:c.left,top:c.top,height:c.height}).show(),TWtid.$border_right.css({left:c.left+c.width,top:c.top,height:c.height}).show(),!0):!1},mousemove:function(a){var a=a||window.event,b=jQuery(document.elementFromPoint(a.clientX,a.clientY));TWtid.$btn_lt||(TWtid.$btn_lt=TWtid.create_button("H","Scroll to A0",function(){TWtid.button_active(jQuery(this))&&TWtid.$cur_block.parent().animate({scrollLeft:0,scrollTop:0})}),TWtid.$btn_lt=TWtid.$btn_lt.add(TWtid.create_button("T","Transpose",
function(){if(!(0<jQuery(this).parent().find(".tedTopt").size())){var a=TWtid.$cur_block,b=TWtid.wrapper_tiddler(a),e=TWtid.element_index(a,b),a=b.tiddler.text.split("\n"),e=TWtid.table_first_line(a,e,b.title),f=TWtid.table_last_line(a,e);TWtid.transpose_table(a,e,f);TWtid.save_text(b.tiddler,a);TWtid.refresh_wrapper(b,!0)}})),TWtid.$btn_lt.appendTo(this).hide());TWtid.inactive_elem(b)||(a=TWtid.block_element(b,a),!a||a.is("[id^=tiddler]")||a.next().is(".title")?TWtid.focus():a&&(!TWtid.$cur_block||
a[0]!=TWtid.$cur_block[0])&&TWtid.focus(a,"on"))},wrapper_closed:function(a){a=a.closest(".sliderPanel");return 0<a.size()&&"none"==a[0].style.display},hasClass:function(a,b,c,d){if(a.hasClass(c))return!0;if(!d||!d.tiddler)return!1;if(config.macros.sortableGridPlugin){a=d.tiddler.text.split("\n");d=TWtid.table_first_line(a,b,d.title);for(b=TWtid.table_last_line(a,d);d<=b;d++)if(/[kK]$/.test(a[d])&&-1<a[d].indexOf(c))return!0}return!1},prepare_table:function(a,b,c,d){if(TWtid.wrapper_closed(a)||0<
a.closest(".tedTable").size())return!1;var e=a.parent();e.is("[id^=tedT]")&&(e=e.parent().parent().find("div[id*=tedTCdiv]"));TWtid.css_size(e.css("line-height"));TWtid.init_table_rows(a,!0,b,c.title,d);config.options.chkTWtidEnabled&&(TWtid.reformat_table(a,b,c.title),a[0].tHead&&(config.tableSorting?TWtid.sortTable||(TWtid.sortTable=config.tableSorting.sortTable,config.tableSorting.sortTable=function(a,b){TWtid.sort(a,b)}):config.macros.sortableGridPlugin&&!TWtid.ts_resortTable&&(TWtid.ts_resortTable=
config.macros.sortableGridPlugin.ts_resortTable,config.macros.sortableGridPlugin.ts_resortTable=function(a){TWtid.sort(a)})));TWtid.fix_rows(a,!0).fix_cols(a,!0);return config.options.chkTWtidEnabled},closest_block:function(a,b,c){for(var d=b.clientX,b=b.clientY+c,e,f=TWtid.element_box(a),f=0>c?0:f.top+f.height,h=b-f;0<c&&0>h||0>c&&0<h;){if(e=document.elementFromPoint(d,b))if((e=TWtid.block_element(jQuery(e)))&&e[0]!=a[0])break;e=null;b+=c;h=b-f}return e&&!e.is(".tagged,.tagInfo,.tidTags")?e:a},closest_blocks:function(a,
b){var c=TWtid.css_size("line-height",a),d=[];d[0]=TWtid.closest_block(a,b,-c)[0];d[1]=TWtid.closest_block(a,b,c)[0];d[2]=a[0];return jQuery.contains(a[0],d[0])||jQuery.contains(a[0],d[1])?jQuery(d):null},is_block_element:function(a){switch(a[0].nodeName){case "DIV":return a.is(".title,.tabset");case "TABLE":return!0;case "LI":case "BLOCKQUOTE":case "PRE":case "H1":case "H2":case "H3":case "H4":case "H5":case "H6":return 0==a.closest("th,td").size()}return!1},block_element:function(a,b){var c=null;
if(TWtid.is_wrapper(a))return b&&(c=TWtid.closest_blocks(a,b)),c?c:a;if(TWtid.is_block_element(a))return a;if(a.is("a.tab"))return a.parent();if((c=TWtid.table_element(a))&&0<c.size())return c;if(a.is("DIV")&&TWtid.$cur_block&&TWtid.$cur_block.is("TABLE")){var c=TWtid.$cur_block.closest(".tedTable"),d=TWtid.element_offset(c);if(b.pageX>=d.left&&b.pageY>=d.top&&b.pageX<=d.left+c.width()&&b.pageY<=d.top+c.height())return TWtid.$cur_block}c=a.closest("h1,h2,h3,h4,h5,h6");if(0<c.size())return c;c=a.closest("li");
if(0<c.size())return c;c=a.closest("blockquote");if(0<c.size())return c;c=a.closest("pre");if(0<c.size())return c;d=null;c=a.closest("span");if(TWtid.is_wrapper(c))return b&&(d=TWtid.closest_blocks(c,b)),d?d:c;c=a.closest("div");return TWtid.is_wrapper(c)?(b&&(d=TWtid.closest_blocks(c,b)),d?d:c):0<c.size()?c:null},apparent_dim:function(a){switch(a[0].nodeName){case "TABLE":config.options.chkTWtidEnabled&&(a=a.parent());case "PRE":case "BLOCKQUOTE":return{W:a.width(),H:a.height()};case "SPAN":case "DIV":if(a.is(".sliderPanel,.tabContents,[tiddler]"))return{W:a.width(),
H:a.height()}}var b=0<a.find("span").size()?a.parent():a,c=jQuery("<span></span>");config.browser.isIE?c.html(a.html()):TWtid.copy_html(c,a);c.appendTo(b);a={W:c.outerWidth(!0),H:c.outerHeight(!0)};c.remove();return a},find_text_nodes:function(a){return a.contents().filter(function(){return 3==this.nodeType})},find_elements:function(a,b,c){var d=c?a.children(b):a.find(b),a=a.is(".viewer")?a:a.find("div.viewer");0<a.size()&&(a=c?a.children("div.sliderPanel \t\t\t\t\t\t\t,div.tabContents \t\t\t\t\t\t\t,span[tiddler]"):
a.find("div.sliderPanel \t\t\t\t\t\t\t,div.tabContents \t\t\t\t\t\t\t,span[tiddler]"),0<a.size()&&(d=d.not(c?a.children(b):a.find(b))));return d},find_tables:function(a){return this.find_elements(a,"table")},find_headers:function(a){return this.find_elements(a,"h1,h2,h3,h4,h5,h6")},find_list_items:function(a){var b=this.find_elements(a,"li").not(".tagged li, .tidTags li"),c=jQuery(b[0]);-1<c.text().indexOf("not tagging")&&(b=b.not(c));return b.not(a.find("td li,th li"))},prepare_elements:function(a,
b){if(0!=a.closest("#displayArea").size()&&b&&b.tiddler&&!store.isShadowTiddler(b.tiddler.title)){var c=b.tiddler.text.split("\n");TWtid.find_tables(a).each(function(a,e){var f=jQuery(e);0==f.closest(".syntaxhighlighter").size()&&TWtid.prepare_table(f,{rIndex:a,dIndex:-1},b,c)})}},OfficialExample:function(a){return a?/^TWtid--Example/.test(a):!1},after_math:function(a){var b=jQuery(a[1]);switch(a[1].nodeName){case "TD":case "TH":a=b.closest("table");TWtid.fix_rows_adjust(a).fix_cols_adjust(a);break;
